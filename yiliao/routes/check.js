...
router.post('/third/query/check2', function (req, res, next) {
    ...
    CoreData.find({"_id": req.body._id}, function (err, doc) {
        if(err == null){
            if(doc.length==0){
                res.json("");
            }else if(doc[0].patientId!=req.body.patientId){
                res.json("");
            }else {
                VerifyCode.findOne({"username": req.body.patientId}, function (err, ver) {
                if(ver != null && ver.verifyCode == req.body.verifyCode){
                    res.json(doc);
                } 
                }); 
            }
